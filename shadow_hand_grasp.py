...
class ShadowHandGrasp(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless,
        ...
        # Initiates the data strcutures on which data will be recorded and saved
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_scales in object_scale_dict.values():
            #print(object_scale_dict.values())
...
# Loads self.data with the data structure being used to record the trajectories

        for i in range(num_envs):
            self.data.append([None, 0, {}])
            i += 1
...
################################################################################################################################################
        ...
        # compute aggregate size, changed the values of max_agg_bodies and max_agg_shapes
        print('self.num_shadow_hand_bodies', self.num_shadow_hand_bodies,'self.num_object_bodies', self.num_object_bodies, 'self.num_shadow_hand_shapes', self.num_shadow_hand_shapes, 'self.num_object_shapes', self.num_object_shapes)
        max_agg_bodies = self.num_shadow_hand_bodies * 5 + 5 * self.num_object_bodies + 1  ## default = 27, 24 + 3
        max_agg_shapes = self.num_shadow_hand_shapes * 5 + 5 * self.num_object_shapes + 1  ## default = 25, 20 + 5
...
# Records the object code for each environment  

            self.data[i][0] = object_code
...
# Section that records and saves the trajectory at each timestep
 
        for i in range(self.num_envs):
            #print(self.progress_buf[i].item(), self.shadow_hand_dof_pos[i].tolist())
            self.data[i][2].update({self.progress_buf[i].item():self.shadow_hand_dof_pos[i].tolist()})
            if self.successes[i] == 1 and self.data[i][1] == 0:
                self.data[i][1] = self.progress_buf[i].item()


        print(self.progress_buf[0].item())

        final_list = []

        if self.progress_buf[0].item() == 200:
            for item in self.data:
                if item[1] != 0:
                    final_list.append(item)
            #print('IF STATEMENT HIT')
            #for i in range(self.num_envs):
                #self.data[i][1] = self.successes[i].item()
                #print('SUCCESSES', self.data[i][1])
            ...
            with open(f'/home/u172702/dexgrasp/UniDexGrasp2/dexgrasp/data/data{len(final_list)}_{self.reset_dof_pos_noise}.pkl', 'wb') as f:
            # protocol=pickle.HIGHEST_PROTOCOL for best efficiency
                pickle.dump(final_list, f, protocol=pickle.HIGHEST_PROTOCOL)
                print('SAVED!')

        self.check_contact_and_print_simple()
################################################################################################################################################
    ...
    def reset(self, env_ids, goal_env_ids):
        ...
        print('these are the env ids', len(env_ids), type(env_ids))
        ...
        pos = self.shadow_hand_default_dof_pos  + self.reset_dof_pos_noise * rand_delta    ####### ADDED THE RANDOMIZATION
        ...
        new_theta = torch_rand_float(-3.14, 3.14, (len(env_ids), 1), device=self.device)[:, 0]
